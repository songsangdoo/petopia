...
<%@page import="com.petopia.board.free.model.FreeBoardCommentTO"%>
<%@page import="com.petopia.board.free.model.FreeBoardFileTO"%>
<%@page import="com.petopia.board.free.model.FreeBoardTO"%>
...
<%
	...
	FreeBoardTO data = (FreeBoardTO)request.getAttribute("data");
	List<FreeBoardFileTO> file_list = (List)request.getAttribute("file_list");
	List<FreeBoardCommentTO> cmt_list = (List)request.getAttribute("cmt_list");
	...
	data.setFb_dt(dt_formatted);
	 
	int fb_seq = data.getFb_seq();
	...
	if(file_list.size() != 0) {
		for(int i = 0; i < file_list.size(); i++){
			...
			sbFileHtml.append("<img src='../../upload/" + file_list.get(i).getFb_file_img_path() + "' class='d-block' width='100%' style='border-radius: 17px;'>");
	...
	for(int i = startNum; i < cmt_list.size() && i < startNum + recordsPerpage; i++){
		...
		String cmt_dt = cmt_list.get(i).getFb_cmt_dt();
		...
		sbCmtHtml.append("<button type='button' class='btn btn-outline-primary' onclick='rec_up(" + fb_seq + ", " + cmt_list.get(i).getFb_cmt_seq() + ")'><img src='../../images/free-icon-thumb-up-8279617.png'>&nbsp;&nbsp;&nbsp;&nbsp;<span>" + cmt_list.get(i).getFb_cmt_rec_cnt() + "</span></button>");
		sbCmtHtml.append("&nbsp;");
		sbCmtHtml.append("<button type='button' class='btn btn-outline-danger' onclick='no_rec_up(" + fb_seq + ", " + cmt_list.get(i).getFb_cmt_seq() + ")'><img src='../../images/free-icon-thumb-down-8279616.png'>&nbsp;&nbsp;&nbsp;&nbsp;<span>" + cmt_list.get(i).getFb_cmt_no_rec_cnt() + "</span></button>");
		if(isCmtWriter || isManager){
			sbCmtHtml.append("&nbsp;");
			sbCmtHtml.append("<button type='button' class='btn btn-outline-danger' onclick='cmt_del(" + cmt_list.get(i).getFb_cmt_seq() + ", " + fb_seq + ")'>삭	제</button>");
		}
		sbCmtHtml.append("</div>");
		sbCmtHtml.append("</div>");
		sbCmtHtml.append("<br>");
		sbCmtHtml.append("<div><p>" + cmt_list.get(i).getFb_cmt_content().replaceAll("<br>", "\n") + "</p></div>");
		sbCmtHtml.append("</td>");
		sbCmtHtml.append("<td>");
		sbCmtHtml.append("</td>");
		sbCmtHtml.append("</tr>");
	...
	if(cmt_cpage == 1){
		...
		cmt_pageSbHtml.append("<li class='page-item'><a class='page-link' href='freeboard_view.do?fb_seq=" + fb_seq + "&cmt_cpage=" + (cmt_cpage - 1) + "'> &lt; </a></li>");
	}
	for(int i = startBlock; i <= endBlock; i++){
		if(i == cmt_cpage){
			cmt_pageSbHtml.append("<li class='page-item'><a class='page-link'>" + i + "</a></li>");
		} else{
			cmt_pageSbHtml.append("<li class='page-item'><a class='page-link' href='freeboard_view.do?fb_seq=" + fb_seq + "&cmt_cpage=" + i + "'>" + i + "</a></li>");
		}
	}
...
window.onload = function(){
	...
	cbtn.onclick = function(){
		...
		if(document.cfrm.fb_cmt_content.value.trim() == ''){
	...
	recBtn.onclick = function(){
		...
		location.href='freeboard_view_rec.do?fb_seq=<%= fb_seq %>';
	}

};

function cmt_del(cmt_seq, fb_seq, m_seq){
	Swal.fire({
		  title: '댓글을 삭제하시겠습니까?',
		  text: '',
		  icon: 'question',
		  ...
		  if (result.isConfirmed) {
			  location.href = 'freeboard_view_cmt_del.do?fb_seq=' + fb_seq + '&fb_cmt_seq=' + cmt_seq;
		  }else{
			  return false;
		  }
	});
...
function post_del(fb_seq){
	Swal.fire({
		  ...
		  if (result.isConfirmed) {
			  location.href='freeboard_delete_ok.do?fb_seq=' + fb_seq;
		  }else{
			  return false;
		  }
	});
...
function rec_up(seq, cmt_seq){
	...
	location.href='freeboard_view_cmt_rec.do?fb_seq=' + seq + '&&fb_cmt_seq=' + cmt_seq;
}
...
function no_rec_up(seq, cmt_seq){
	...
	location.href='freeboard_view_cmt_no_rec.do?fb_seq=' + seq + '&&fb_cmt_seq=' + cmt_seq;
}
</script>  
</head>
			...
			<!--게시판-->
				<div class='container' id="board-view" style='width:800px;'>
					...
					<tr>
						...
						<h4><%= data.getFb_subject() %></h4>
						<div class="d-flex justify-content-end text-black">
						  <div class="p-2 bd-highlight">작성일 : <%= data.getFb_dt() %></div>
						  <div class="p-2 bd-highlight">조회수 : <%= data.getFb_hit() %></div>
						  <div class="p-2 bd-highlight">추천수 : <%= data.getFb_rec_cnt() %></div>
						</div>
						</td>
					</tr>
					<tr>
						...
						<br>
							...
							<p class="lead"><%= data.getFb_content() %></p>
							</div>
					...
					<div class='cmt-btn-group' align="right">
						<button type="button" class="btn btn-outline-primary" style="cursor: pointer;" onclick="location.href='freeboard_list.do'" >목	록</button>
						<c:if test="<%= !isWriter %>">
						<button type='button' class="btn btn-outline-primary" id = 'recBtn'>추 천</button>
						</c:if>
						<c:if test="<%= isWriter %>">
							<button type="button" class="btn btn-outline-primary" style="cursor: pointer;" onclick="location.href='freeboard_modify.do?fb_seq=<%= fb_seq %>'">수    정</button>
						</c:if>
						<c:if test="<%= isWriter || isManager %>">
							<button type="button" class="btn btn-outline-danger" style="cursor: pointer;" onclick="post_del(<%= fb_seq %>)">삭	제</button>
						</c:if>
					</div>
					<!-- 글 수정, 삭제버튼 -->
					...
					<table class="table" >
					<tr>
					<td><strong>댓글수</strong> &nbsp;&nbsp; <%= data.getFb_cmt_cnt() %>&nbsp;&nbsp;&nbsp;&nbsp;<a href="freeboard_view.do?fb_seq=<%= fb_seq %>&&order_flag=latest">최신순</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="freeboard_view.do?fb_seq=<%= fb_seq %>&&order_flag=rec">추천순</a></td>
					<td></td>
					</tr>
					...
					<div class="container w-80 mt-2">
						<form action="freeboard_view_cmt_write.do" method="post" name="cfrm">
						<input type="hidden" value="<%= fb_seq %>" name="fb_seq">
						...
						<tr>
							<td>
								<div class="input-group mb-3">
									...
									<textarea class="form-control" name="fb_cmt_content" rows="5" style="resize: none;"></textarea>
								</div>
								<div align="right">
