...
<%
	if(request.getAttribute("rec_insert") != null){
		...
		if(rec_insert == 0){ %>
		Swal.fire('게시물을 추천했습니다', '', 'success').then(function(){location.href = 'freeboard_view.do?fb_seq=<%= fb_seq %>';});
		<%}else{%>
	...
	if(request.getAttribute("cmt_write_flag") != null){
		...
		if(cmt_write_flag == 0){ %>
		Swal.fire('댓글을 작성했습니다', '', 'success').then(function(){location.href = 'freeboard_view.do?fb_seq=<%= fb_seq %>';});
		<%}else{%>
		Swal.fire('댓글을 작성에 실패했습니다', '', 'error').then(function(){location.href = 'freeboard_view.do?fb_seq=<%= fb_seq %>';});
		<%}
	}
	...
	if(request.getAttribute("rec_cmt_check_flag") != null){
		...
		Swal.fire('댓글을 추천했습니다', '', 'success').then(function(){location.href = 'freeboard_view.do?fb_seq=<%= fb_seq %>';});
		<%}else{%>
	...
	if(request.getAttribute("rec_cmt_del_check_flag") != null){
		...
		Swal.fire('댓글을 삭제했습니다', '', 'success').then(function(){location.href = 'freeboard_view.do?fb_seq=<%= fb_seq %>';});
		<%}else{%>
		Swal.fire('댓글을 삭제에 실패했습니다', '', 'error').then(function(){location.href = 'freeboard_view.do?fb_seq=<%= fb_seq %>';});
		<%}
	}%>
