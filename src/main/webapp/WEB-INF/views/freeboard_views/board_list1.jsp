...
<%
	...
	List<FreeBoardTO> datas = (List)request.getAttribute("datas");
	...
	for (int i = startNum; i < datas.size() && i < startNum + recordsPerpage; i++) {
		String dt = datas.get(i).getFb_dt();
		...
		sbHtml.append("<tr>");
		sbHtml.append("<td>&nbsp;</td>");
		sbHtml.append("<td align='center'>" + datas.get(i).getFb_seq() + "</td>");
		sbHtml.append("<td>");
		sbHtml.append("<a href='freeboard_view.do?fb_seq=" + datas.get(i).getFb_seq() + "' style='color: black;'>");
		sbHtml.append("&nbsp;&nbsp;&nbsp;&nbsp;<strong>" + datas.get(i).getFb_subject() + "</strong>");
		if(datas.get(i).getFb_cmt_cnt() != 0){
			sbHtml.append("&nbsp;&nbsp;<span>[" + datas.get(i).getFb_cmt_cnt() + "]</span>");
		}
		if(newIcon){
		...
		sbHtml.append("<td align='center'>" + datas.get(i).getFb_hit() + "</td>");
		sbHtml.append("<td align='center'>" + datas.get(i).getFb_rec_cnt() + "</td>");
		sbHtml.append("</tr>");
	}
	...
	if(cpage == 1){
		...
		pageSbHtml.append("<li class='page-item'><a class='page-link' href='freeboard_list.do?cpage=" + (cpage - 1) + "'> &lt; </a></li>");
	}
	for(int i = startBlock; i <= endBlock; i++){
		if(i == cpage){
			pageSbHtml.append("<li class='page-item active'><a class='page-link'>" + i + "</a></li>");
		} else{
			pageSbHtml.append("<li class='page-item'><a class='page-link' href='freeboard_list.do?cpage=" + i + "'>" + i + "</a></li>");
		}
	}
	...
	<main id="main">
		<section>
			<div class="container" data-aos="fade-up">
				  		...
				  		<td>
							<div class="dropdown">
							  ...
							  <ul class="dropdown-menu">
							    <li><a class="dropdown-item" href="freeboard_list.do?order_flag=latest">최신순</a></li>
							    <li><a class="dropdown-item" href="freeboard_list.do?order_flag=rec">추천순</a></li>
							    <li><a class="dropdown-item" href="freeboard_list.do?order_flag=cmt">댓글순</a></li>
							    <li><a class="dropdown-item" href="freeboard_list.do?order_flag=hit">조회수순</a></li>
							  </ul>
							</div>
						</td>
				  	</tr>
				...
				<div align="right">
					<input type="button" value="글 쓰기" class="btn btn-outline-primary" style="cursor: pointer; border:none;" onclick="location.href='freeboard_write.do'" />&nbsp;&nbsp;&nbsp;&nbsp;
				</div>
				</c:if>	
				<!-- 버튼 끝 -->	
			...
			<!-- 검색 -->
			<form action="freeboard_list.do" method="post">
