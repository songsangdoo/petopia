...
<%
	...
	List<GroupBoardTO> datas = (List)request.getAttribute("datas");
	...
	for (int i = startNum; i < datas.size() && i < startNum + recordsPerpage; i++) {
		String dt = datas.get(i).getGR_DATE();
		...
		sbHtml.append("<tr>");
		sbHtml.append("<td>&nbsp;</td>");
		sbHtml.append("<td align='center'>" + datas.get(i).getGRB_SEQ() + "</td>");
		sbHtml.append("<td>");
		sbHtml.append("<a href='mygroup_board_view.do?gr_seq="+ group_seq + "&grb_seq="+datas.get(i).getGRB_SEQ()+"' style='color: black;'>");
		sbHtml.append("&nbsp;&nbsp;&nbsp;&nbsp;<strong>" + datas.get(i).getGR_SUBJECT() + "</strong>");
		if(datas.get(i).getGR_COMMENT() != 0){
			sbHtml.append("&nbsp;&nbsp;<span>[" + datas.get(i).getGR_COMMENT() + "]</span>");
		}
		if(newIcon){
		...
		sbHtml.append("<td align='center'>" + datas.get(i).getGR_HIT() + "</td>");
		sbHtml.append("<td align='center'>" + datas.get(i).getGR_GOOD() + "</td>");
		sbHtml.append("</tr>");
	} 
	...
	if(cpage == 1){
		...
		pageSbHtml.append("<li class='page-item'><a class='page-link' href='mygroup_board_list.do?gr_seq="+group_seq+"&cpage=" + (cpage - 1) + "'> &lt; </a></li>");
	}
	for(int i = startBlock; i <= endBlock; i++){
		if(i == cpage){
			pageSbHtml.append("<li class='page-item'><a class='page-link'>" + i + "</a></li>");
		} else{
			pageSbHtml.append("<li class='page-item'><a class='page-link' href='mygroup_board_list.do?gr_seq="+group_seq+"&cpage=" + i + "'>" + i + "</a></li>");
		}
	}
	...
	if(cpage == totalPage){
		...
		pageSbHtml.append("<li class='page-item'><a class='page-link' href='mygroup_board_list.do?cpage=" + (cpage + 1) + "'> &gt; </a></li>");
	}
...
<body>
	...
	<main id="main">
			<div class="container">
				  		...
				  		<td>
							<div class="dropdown">
							  ...
							  <ul class="dropdown-menu">
							    <li><a class="dropdown-item" href="mygroup_board_list.do?order_flag=latest&gr_seq=<%=group_seq %>">최신순</a></li>
							    <li><a class="dropdown-item" href="mygroup_board_list.do?order_flag=rec&gr_seq=<%=group_seq %>">추천순</a></li>
							    <li><a class="dropdown-item" href="mygroup_board_list.do?order_flag=cmt&gr_seq=<%=group_seq %>">댓글순</a></li>
							    <li><a class="dropdown-item" href="mygroup_board_list.do?order_flag=hit&gr_seq=<%=group_seq %>">조회수순</a></li>
							  </ul>
							</div>
						</td>
				  	</tr>
				...
				<div align="right">
					<input type="button" value="글 쓰기" class="btn btn-outline-primary" style="cursor: pointer; border:none;" onclick="location.href='mygroup_board_write.do?gr_seq=<%=group_seq %>'" />&nbsp;&nbsp;&nbsp;&nbsp;
				</div>
				</c:if>	
				<!-- 버튼 끝 -->	
			...
			<!-- 검색 -->
			<form action="mygroup_board_list.do" method="post">
			<input type="hidden" name="gr_seq" value="<%=group_seq%>">
