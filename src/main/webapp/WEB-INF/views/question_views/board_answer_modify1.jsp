...
<%
	...
	QuestionAnswerTO ans_data = (QuestionAnswerTO)request.getAttribute("ans_data");
	...
	for(int i = 0; file_list != null && i < file_list.size(); i++){
		...
		sbFileHtml.append("<img src='../../upload/" + file_list.get(i).getQ_file_img_path() + "' class='d-block' width=400 style='border-radius: 17px;'/>");
	...
	int q_ans_seq = ans_data.getQ_ans_seq();
%>
<!DOCTYPE html>
...
<body>
	...
	<main id="main">
		...
		<form action="question_ans_modify_ok.do" method="post" name="wfrm" enctype="multipart/form-data">
		<input type="hidden" name="q_seq" value="<%= q_seq %>">
		<input type="hidden" name="q_ans_seq" value="<%= q_ans_seq %>">
		<div class="input-group mb-3">
			<span class="input-group-text">답 변</span>
			<textarea class="form-control" name="content" rows="10" style="resize: none;"><%= ans_data.getQ_ans_content().replaceAll("<br>", "\n") %></textarea>
		</div>
