...
<%@page import="com.petopia.board.album.model.AlbumCommentTO"%>
<%@page import="com.petopia.board.album.model.AlbumFileTO"%>
...
<%
	...
	List<AlbumFileTO> file_list = (List)request.getAttribute("file_list");
	List<AlbumCommentTO> cmt_list = (List)request.getAttribute("cmt_list");
	...
	data.setAb_dt(dt_formatted);
	 
	int ab_seq = data.getAb_seq();
	
	System.out.println(ab_seq);
	...
	if(file_list.size() != 0) {
		for(int i = 0; i < file_list.size(); i++){
			...
			sbFileHtml.append("<img src='../../upload/" + file_list.get(i).getAb_file_img_path() + "' class='d-block' width='100%' style='border-radius: 17px;'>");
	...
	for(int i = startNum; i < cmt_list.size() && i < startNum + recordsPerpage; i++){
		...
		String cmt_dt = cmt_list.get(i).getAb_cmt_dt();
		...
		sbCmtHtml.append("<button type='button' class='btn btn-outline-primary ' onclick='rec_up(" + ab_seq + ", " + cmt_list.get(i).getAb_cmt_seq() + ")'><img src='../../images/free-icon-thumb-up-8279617.png'>&nbsp;&nbsp;&nbsp;&nbsp;<span>" + cmt_list.get(i).getAb_cmt_rec_cnt() + "</span></button>");
		sbCmtHtml.append("&nbsp;&nbsp;");
		sbCmtHtml.append("<button type='button' class='btn btn-outline-danger' onclick='no_rec_up(" + ab_seq + ", " + cmt_list.get(i).getAb_cmt_seq() + ")'><img src='../../images/free-icon-thumb-down-8279616.png'>&nbsp;&nbsp;&nbsp;&nbsp;<span>" + cmt_list.get(i).getAb_cmt_no_rec_cnt() + "</span></button>");
		sbCmtHtml.append("&nbsp;&nbsp;");
		if(isCmtWriter || isManager){
			sbCmtHtml.append("<button type='button' class='btn btn-outline-danger' onclick='cmt_del(" + cmt_list.get(i).getAb_cmt_seq()+ ", " + ab_seq + ", " + m_seq + ")'>삭	제</button>");
		}
		sbCmtHtml.append("</div>");
		sbCmtHtml.append("</div>");
		sbCmtHtml.append("<br>");
		sbCmtHtml.append("<div><p>" + cmt_list.get(i).getAb_cmt_content() + "</p></div>");
		sbCmtHtml.append("</td>");
		sbCmtHtml.append("<td>");
		sbCmtHtml.append("</td>");
		sbCmtHtml.append("</tr>");
	...
	if(cmt_cpage == 1){
		...
		cmt_pageSbHtml.append("<li class='page-item'><a class='page-link' href='album_view.do?ab_seq=" + ab_seq + "&cmt_cpage=" + (cmt_cpage - 1) + "'> &lt; </a></li>");
	}
	for(int i = startBlock; i <= endBlock; i++){
		if(i == cmt_cpage){
			cmt_pageSbHtml.append("<li class='page-item'><a class='page-link'>" + i + "</a></li>");
		} else{
			cmt_pageSbHtml.append("<li class='page-item'><a class='page-link' href='album_view.do?ab_seq=" + ab_seq + "&cmt_cpage=" + i + "'>" + i + "</a></li>");
		}
	}
...
window.onload = function(){
	...
	cbtn.onclick = function(){
		...
		if(document.cfrm.ab_cmt_content.value.trim() == ''){
	...
	recBtn.onclick = function(){
		...
		location.href='album_view_rec.do?ab_seq=<%= ab_seq %>';
	}

};

function cmt_del(cmt_seq, ab_seq, m_seq){
	Swal.fire({
		  title: '댓글을 삭제하시겠습니까?',
		  text: '',
		  icon: 'question',
		  ...
		  if (result.isConfirmed) {
			  location.href = 'album_view_cmt_del.do?ab_seq=' + ab_seq + '&ab_cmt_seq=' + cmt_seq;
		  }else{
			  return false;
		  }
	});
...
function post_del(ab_seq){
	Swal.fire({
		  ...
		  if (result.isConfirmed) {
			  location.href='album_delete_ok.do?ab_seq=' + ab_seq;
		  }else{
			  return false;
		  }
	});
...
function rec_up(seq, cmt_seq){
	...
	location.href='album_view_cmt_rec.do?ab_seq=' + seq + '&&ab_cmt_seq=' + cmt_seq;
}
...
function no_rec_up(seq, cmt_seq){
	...
	location.href='album_view_cmt_no_rec.do?ab_seq=' + seq + '&&ab_cmt_seq=' + cmt_seq;
}
</script>
		...
		<div class="container mt-3"> 
			...
			<div class='container' id="board-view" style='width:800px;'>
					...
					<tr>
						...
						<h4><%= data.getAb_subject() %></h4>
						<div class="d-flex justify-content-end text-black">
						  <div class="p-2 bd-highlight">작성일 : <%= data.getAb_dt() %></div>
						  <div class="p-2 bd-highlight">조회수 : <%= data.getAb_hit() %></div>
						  <div class="p-2 bd-highlight">추천수 : <%= data.getAb_rec_cnt() %></div>
						</div>
						</td>
					</tr>
					<tr>
						...
						<br>
							...
							<div id="carouselExampleFade" class="carousel slide carousel-fade">
							  <div class="carousel-inner">
							    <div class="carousel-item active">
							      <img src="../../upload/<%= data.getAb_rep_img_path() %>" width='100%' class="d-block" style="border-radius: 17px;">
							    </div>
							    <%= sbFileHtml %>
							  </div>
							...
							<p class="lead"><%= data.getAb_content() %></p>
							</div>
					...
					<div class='cmt-btn-group' align="right">
						<button type="button" class="btn btn-outline-primary" style="cursor: pointer;" onclick="location.href='album_list.do'" >목	록</button>
						<c:if test="<%= !isWriter %>">
						<button type='button' class="btn btn-outline-primary" id = 'recBtn'>추 천</button>
						</c:if>
						<c:if test="<%= isWriter %>">
							<button type="button" class="btn btn-outline-primary" style="cursor: pointer;" onclick="location.href='album_modify.do?ab_seq=<%= ab_seq %>'">수    정</button>
						</c:if>
						<c:if test="<%= isWriter || isManager %>">
							<button type="button" class="btn btn-outline-danger" style="cursor: pointer;" onclick="post_del(<%= ab_seq%>)">삭	제</button>
						</c:if>
					</div>
					<!-- 글 수정, 삭제버튼 -->
					...
					<table class="table">
					<tr>
					<td><strong>댓글수</strong> &nbsp;&nbsp; <%= data.getAb_cmt_cnt() %>&nbsp;&nbsp;&nbsp;&nbsp;<a href="album_view.do?ab_seq=<%= ab_seq %>&&order_flag=latest">최신순</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="album_view.do?ab_seq=<%= ab_seq %>&&order_flag=rec">추천순</a></td>
					<td></td>
					</tr>
					...
					<div class="container mt-2">
						<form action="album_view_cmt_write.do" method="post" name="cfrm">
						<input type="hidden" value="<%= ab_seq %>" name="ab_seq">
						...
						<tr>
							<td>
								<div class="input-group mb-3">
									...
									<textarea class="form-control" name="ab_cmt_content" rows="5" style="resize: none;"></textarea>
								</div>
								<div align="right">
